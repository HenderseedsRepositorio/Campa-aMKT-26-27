# Post 03 — Institucional: el equipo detrás de la recomendación

- **Semana:** 2026-W27 · publicación tentativa: 2026-07-04
- **Pilar:** institucional / cercanía
- **Formato:** placa crema (1080×1350) → pieza HTML: `docs/placas/w27-post-03-institucional.html` · (opción: reemplazar por foto real del equipo)
- **Objetivo:** humanizar la marca; mostrar que detrás hay personas de la zona que conocen los campos; reforzar la confianza y el vínculo cercano. Invitar a pasar por la oficina.
- **Pauta:** no (orgánico, bajo presupuesto si se quiere amplificar alcance)

## Copy (Instagram/Facebook)
> Detrás de cada recomendación hay alguien que conoce tu lote.
>
> No somos un 0800 ni un catálogo. Somos de Henderson, recorremos los campos de Daireaux y Bolívar, y cuando te decimos "este híbrido va acá", es porque lo vimos funcionar en ambientes como el tuyo.
>
> Eso es ser parte de la red RED.IN de Nidera Semillas: genética de punta con respaldo técnico local.
>
> Pasá por la oficina, llamanos o escribinos. La precampaña 26/27 se charla mejor con un mate de por medio.
>
> 📲 Coordiná una visita por WhatsApp.

## Pieza visual
**Foto real** del equipo Henderseeds (en la oficina, en un campo, o en una jornada). Si no hay foto disponible todavía: placa **crema** (`#F6F4EE`) con texto navy.
- Si es foto: overlay leve oscuro abajo para texto. Nombre del equipo o "Equipo Henderseeds · Henderson" en DM Mono, blanco, abajo.
- Logo Henderseeds + sello RED.IN Nidera.
- Tono cálido, no corporativo. Que se note que son personas reales de la zona.
- `[COMPLETAR: necesitamos la foto del equipo o de la oficina — pedirla a Henderseeds]`

## Hashtags (máx 8)
#Henderseeds #Nidera #REDIN #Henderson #Daireaux #Bolivar #OesteBonaerense #Agro

## CTA
📲 Coordiná una visita o escribinos: wa.me/5492314530691

## Fuentes de los datos usados
- Datos de zona y posicionamiento como RED.IN: `contexto/empresa.md`, `contexto/zona-y-audiencia.md`.
- No se hace ninguna afirmación de rendimiento.

## Estado
- [x] ✍️ Borrador
- [ ] ✅ Aprobado
- [ ] 📤 Publicado → link:
- [ ] 📣 Pautado → brief:
